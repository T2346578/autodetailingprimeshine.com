document.addEventListener('DOMContentLoaded', function() {
    // Native date input: limit to current calendar year (same as old Flatpickr behavior)
    const datePicker = document.getElementById('date-picker');
    if (datePicker && datePicker.type === 'date') {
        const y = new Date().getFullYear();
        datePicker.min = y + '-01-01';
        datePicker.max = y + '-12-31';
    }
    ...
    modalForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            if (response.ok) {
                ...
                window.dataLayer.push({ event: 'leadFormSubmitted' });
