    ...
    const fpDate = flatpickr(datePicker, {
        ...
        onReady: function () {
            if (datePicker) {
                ...
                datePicker.style.boxSizing = 'border-box';
            }
        },
    });

    // Open date picker when clicking anywhere on the date form-group
    if (fpDate && datePicker) {
        const dateFormGroup = datePicker.closest('.form-group');
        if (dateFormGroup) {
